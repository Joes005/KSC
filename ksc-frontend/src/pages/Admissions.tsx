...
const ELIGIBILITY_SUMMARY = [
  {
    level: "UG Programmes",
    detail:
      "A pass in Higher Secondary Examination (10+2) or equivalent / 3-year Diploma from a recognized board or institution.",
  },
  {
    level: "PG Programmes",
    detail:
      "A relevant Bachelor's degree from a recognized university. Specific prerequisites are available under each course on the Academic page.",
  },
  {
    level: "Diploma / Certificate / Vocational",
    detail:
      "A pass in Higher Secondary Examination (10+2) or equivalent from a recognized board.",
  },
  {
    level: "Distance & Open learning",
    detail:
      "Programmes are UGC-DEB recognised where applicable; no regular attendance required — study comfortably with our complete guidance.",
  },
];
...
export function Admissions() {
  ...
  const eligibilityData = (pages?.admissions?.eligibility_summary ||
    ELIGIBILITY_SUMMARY) as any[];
  const bannerImage =
    (pages?.admissions?.banner as any)?.image ||
    "/assets/campus/convocation-alagappa.jpg";
  const sectionHeadings = ((pages?.admissions?.section_headings as any) ||
    {}) as Record<
    string,
  ...
  const applicationFields = (
    Array.isArray(pages?.admissions?.application_fields) &&
    (pages!.admissions!.application_fields as any[]).length
      ? pages!.admissions!.application_fields
      : ADMISSIONS_FORM_FIELDS
  ) as typeof ADMISSIONS_FORM_FIELDS;
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden border-b border-ksc-red/20 bg-ksc-navy/5 py-16 lg:py-20 text-ksc-navy">
        ...
        {/* Decorative Grid */}
        <div
          className="absolute inset-0 bg-grid-slate-800/[0.04] bg-[bottom_1px_center] pointer-events-none"
          style={{
        ...
        <div className="container-site relative z-10">
          <p className="section-kicker text-ksc-red bg-white inline-block px-3 py-1 rounded-md mb-2 shadow-sm">
            {headerData.kicker || "Admissions"}
          </p>
      ...
      <section className="relative overflow-hidden bg-slate-50/50 bg-dot-pattern py-10 lg:py-16 border-b border-slate-100 reveal-section">
        ...
        <div className="container-site relative z-10">
          <SectionHeading
            kicker={stepsHeading.kicker || "How to Apply"}
            title={stepsHeading.title || "Your admission journey in four steps"}
          />
          <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mt-8 sm:mt-12">
            {stepsData.map((step, i) => {
              ...
              return (
                <div
                  key={step.step || i}
                  className="card-hover group p-4 sm:p-6 text-center opacity-0 translate-y-12 transition-all duration-700 [.is-visible_&]:opacity-100 [.is-visible_&]:translate-y-0"
      ...
      <section className="relative bg-white py-10 lg:py-16 reveal-section overflow-hidden">
        <div
          className="absolute top-1/2 left-0 w-[500px] h-[500px] bg-ksc-red/5 rounded-full blur-[120px] pointer-events-none animate-ambient-drift"
        ...
        <div className="container-site max-w-5xl mx-auto relative z-10">
          <div>
            <SectionHeading
              ...
              title={
                downloadsHeading.title ||
                "University prospectus & admission details"
              }
              subtitle={
                downloadsHeading.subtitle ||
                "Bring the flyers and prospectus below, or download them here. All four universities admit through our centre."
              }
            />
            <div className="mt-12 grid gap-5 sm:grid-cols-2 opacity-0 scale-95 transition-all duration-700 [.is-visible_&]:opacity-100 [.is-visible_&]:scale-100 delay-200">
              {UNIVERSITIES.map((uni) => {
                ...
                const pdfHref =
                  uni.exam.syllabusUrl || legacyFallback[uni.id] || "";
                ...
                return (
                  ...
                  >
                    ...
                    <span>
                      <span className="block font-black text-ksc-navy uppercase text-sm">
                        {uni.name}
                      </span>
                      <span className="text-xs font-bold text-slate-600 tracking-widest">
                        {hasPdf
                          ? `Prospectus — ${uni.academicYear} (PDF)`
                          : "Enquire at Centre"}
                      </span>
                    </span>
                  </a>
            ...
            <p className="mt-10 text-center text-sm font-bold text-slate-800 max-w-2xl mx-auto bg-slate-50 border-2 border-slate-100 p-4 rounded-xl">
              Not sure which programme suits you? Talk to our counsellors —
              we'll point you to the right course and documents before you
              apply.
            </p>
          </div>
        </div>
      ...
      <section className="relative bg-slate-50/80 bg-dot-pattern py-10 lg:py-16 border-t border-slate-100 reveal-section overflow-hidden">
        <div
          className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-ksc-yellow/5 rounded-full blur-[100px] pointer-events-none animate-ambient-drift"
          style={{ animationDelay: "-10s" }}
        />
        <div className="container-site relative z-10">
          <SectionHeading
            kicker={eligibilityHeading.kicker || "Eligibility at a Glance"}
            title={eligibilityHeading.title || "Who can apply"}
          />
          <div className="mx-auto grid max-w-4xl gap-5 sm:grid-cols-2 mt-12">
            {eligibilityData.map((item, i) => (
              <div
                key={item.level}
                className="card-hover p-6 opacity-0 translate-y-12 transition-all duration-700 [.is-visible_&]:opacity-100 [.is-visible_&]:translate-y-0"
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                <div className="flex items-center gap-3">
                  <BadgeCheck className="h-6 w-6 shrink-0 text-ksc-red" />
                  <h3 className="font-black uppercase text-ksc-navy">
                    {item.level}
                  </h3>
                </div>
          ...
          <p className="mx-auto mt-8 max-w-4xl text-center text-sm font-bold text-slate-700">
            Final eligibility is decided by the respective university. Not sure
            you qualify?{" "}
            <Link
              to="/contact"
              className="font-black text-ksc-royal hover:text-ksc-red hover:underline"
            >
              Ask our counsellors
            </Link>{" "}
        ...
        <div className="container-site relative z-10 grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <SectionHeading
              ...
              subtitle={
                applyFormHeading.subtitle ||
                "Share your details and we'll call you back with the exact documents, fees and next steps for your chosen course."
              }
            />
          ...
          <aside className="opacity-0 translate-x-[20px] transition-all duration-700 [.is-visible_&]:opacity-100 [.is-visible_&]:translate-x-0 delay-300">
            <SectionHeading
              align="left"
              kicker={universitiesSidebarHeading.kicker || "Universities"}
              title={universitiesSidebarHeading.title || "Programmes available"}
            />
            <ul className="space-y-4 mt-8">
              {UNIVERSITIES.map((uni) => (
                <li key={uni.id}>
                  ...
                  >
                    <span>
                      <span className="block font-black uppercase text-ksc-navy">
                        {uni.name}
                      </span>
            ...
            <div className="glass-panel mt-8 p-8">
              ...
              <h3 className="mt-4 text-lg font-black uppercase text-ksc-navy">
                Document Required for Admission
              </h3>
              <ul className="mt-4 space-y-2 text-sm font-bold text-slate-800">
                <li className="flex gap-2 items-start">
                  <span className="text-ksc-red font-bold">·</span> 10th / 12th
                  mark sheets &amp; certificates
                </li>
                <li className="flex gap-2 items-start">
                  <span className="text-ksc-red font-bold">·</span> Degree
                  certificates (for PG applicants)
                </li>
                <li className="flex gap-2 items-start">
                  <span className="text-ksc-red font-bold">·</span>{" "}
                  Passport-size photographs
                </li>
                <li className="flex gap-2 items-start">
                  <span className="text-ksc-red font-bold">·</span> Aadhaar / ID
                  proof
                </li>
                <li className="flex gap-2 items-start">
                  <span className="text-ksc-red font-bold">·</span> Community
